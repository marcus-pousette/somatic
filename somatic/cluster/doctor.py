...
from somatic.cluster.errors import PreflightError
from somatic.cluster.hosts import Host, validate_model_id
...
def preflight(host: Host, model_id: str) -> list[tuple[str, CheckResult]]:
    ...
    reach = ssh.run(host, "echo somatic-ok", timeout=12.0)
    checks.append((
        "ssh",
        CheckResult(reach.ok and "somatic-ok" in reach.stdout,
