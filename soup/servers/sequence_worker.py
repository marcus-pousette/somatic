...
from soup.sequence_model.execution import SimulatedRuntimeAdapter
from soup.sequence_model.interfaces import CalibrationProfile, ModelManifest, ModelUnit, Precision, ResourceProfile, UnitKind
...
from soup.sequence_model.boundary_codec_backend import (
...
from soup.sequence_model.qwen_real import QwenCacheForwardResult, QwenShardTrace, QwenWorkerLayerRuntime
from soup.sequence_model.tensor_execution import TensorRuntimeAdapter
from soup.sequence_model.telemetry import SequenceWorkerMetrics, collect_sequence_worker_metrics
from soup.sequence_model.tensors import (
...
from soup.sequence_model.transformers_qwen import QwenGenerationResult, TransformersQwenAdapter
from soup.servers.provider_probe import (
...
def _qwen_binary_response(
    *,
    ...
    response_headers["x-soup-response-frame-encode-ms"] = f"{response_frame_encode_ms:.9f}"
    return Response(
        content=content,
        media_type=TENSOR_FRAME_MEDIA_TYPE,
...
def _worker_telemetry_headers(
    *,
    ...
    headers = {"x-soup-worker-cpu-seconds": f"{process_cpu_seconds:.9f}"}
    if rss_bytes is not None:
        headers["x-soup-worker-rss-bytes"] = str(int(rss_bytes))
    return headers
