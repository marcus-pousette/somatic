"""Computer Soup (Python package `soup`).
...
def __getattr__(name: str):
    # Lazy so `import soup` stays cheap (no torch/transformers import cost).
    if name in ("Cluster", "Host"):
        from soup.cluster import Cluster, Host

        return {"Cluster": Cluster, "Host": Host}[name]
    raise AttributeError(f"module 'soup' has no attribute {name!r}")
