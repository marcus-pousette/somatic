...
from soup.cluster.capacity import LaunchPlan, PlanEntry, fit_ranges, probe_free_ram
from soup.cluster.errors import WorkerFailedError
...
class Supervisor:
    ...
    def worker_specs(self, plan: LaunchPlan):
        ...
        from soup.serving.cluster_engine import WorkerSpec
    ...
    def _start_server(self, plan: LaunchPlan) -> None:
        ...
        from soup.serving.cluster_engine import ClusterEngine
    ...
    def _log_path(self, entry: PlanEntry, index: int) -> str:
        if entry.host.is_local:
            from soup.cluster.runstate import STATE_ROOT

            d = STATE_ROOT / self.run_id
            d.mkdir(parents=True, exist_ok=True)
            return str(d / f"w{index}.log")
        return f".soup/{self.run_id}-w{index}.log"
